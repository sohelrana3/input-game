      ...
      <div className='container mx-auto flex py-40 h-screen'>
        ...
        <div className='w-1/2 pl-14'>
          ...
          <ol className='list-decimal'>
            {list.map((item, index)=>(
              <li key={index} className='text-white font-medium text-xl'>{item.list} <button className='border border-white text-red-500 px-4 text-base' onClick={(id)=> handledel(item.id)}>Delet</button> <button className='border border-white text-red-500 px-4 text-base' onClick={openModal}>Edit</button></li>
            ))}
          ...
          <div>
            ...
            >
              <div className='relative'>
                <h2 ref={(_subtitle) => (subtitle = _subtitle)}>Hello Shawon sir</h2>
                <button className='absolute top-0 right-0 border border-red-500 px-4' onClick={closeModal}>close</button>
                  <h2 className='text-center mb-3 text-2xl font-bold'>Total: {total}</h2>
                <div>
                  <div className='mb-5'>
                  <input ref={addref} onChange={(e)=> setadd(e.target.value)} className='border border-black mx-2' placeholder='Add  +' />
                  <input ref={Divisionref} onChange={(e)=> setDivision(e.target.value)} className='border border-black mx-2' placeholder='Division  /' />
                  </div>
                  <div>
                  <input ref={Minusref} onChange={(e)=> setMinus(e.target.value)} className='border border-black mx-2' placeholder='Minus  -' />
                  <input ref={Multiplicationref} onChange={(e)=> setMultiplication(e.target.value)} className='border border-black mx-2' placeholder='Multiplication  *'/>
                  </div>
                  
                </div>
                <div>
                  <button onClick={handlebutton} className='w-full bg-[#22D3EE] mt-4 py-4 rounded-full'>Submit</button>
                </div>
